 ...
 *****************************************************************************/
struct Output *
output_create(const struct sysscan *sysscan, unsigned thread_index)
{
    struct Output *out;
    unsigned i;
    ...
    /*
     * Copy the configuration information from the 'sysscan' structure.
     */
    out->rotate.period = sysscan->output.rotate.timeout;
    out->rotate.offset = sysscan->output.rotate.offset;
    out->rotate.filesize = sysscan->output.rotate.filesize;
    out->redis.port = sysscan->redis.port;
    out->redis.ip = sysscan->redis.ip;
    out->is_banner = sysscan->is_banners;
    out->is_gmt = sysscan->is_gmt;
    out->is_interactive = sysscan->output.is_interactive;
    out->is_show_open = sysscan->output.is_show_open;
    out->is_show_closed = sysscan->output.is_show_closed;
    out->is_show_host = sysscan->output.is_show_host;
    out->is_append = sysscan->output.is_append;
    out->xml.stylesheet = duplicate_string(sysscan->output.stylesheet);
    out->rotate.directory = duplicate_string(sysscan->output.rotate.directory);
    if (sysscan->nic_count <= 1)
        out->filename = duplicate_string(sysscan->output.filename);
    else
        out->filename = indexed_filename(sysscan->output.filename, thread_index);

    for (i=0; i<8; i++) {
        out->src[i] = sysscan->nic[i].src;
    }
     ...
     */
    out->format = sysscan->output.format;
    switch (out->format) {
    case Output_List:
     ...
     */
    if (sysscan->output.filename[0] && out->funcs != &null_output) {
        FILE *fp;

        fp = open_rotate(out, sysscan->output.filename);
        if (fp == NULL) {
            perror(sysscan->output.filename);
            exit(1);
        }
     ...
     */
    if (sysscan->output.rotate.timeout == 0) {
