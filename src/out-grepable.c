...
grepable_out_open(struct Output *out, FILE *fp)
{
    ...
    fprintf(fp, "# sysscan " sysscan_VERSION " scan initiated %s\n", 
                timestamp);

    count = count_type(&out->sysscan->ports, Templ_TCP);
    fprintf(fp, "# Ports scanned: TCP(%u;", count);
    if (count)
        print_port_list(&out->sysscan->ports, Templ_TCP, fp);

    count = count_type(&out->sysscan->ports, Templ_UDP);
    fprintf(fp, ") UDP(%u;", count);
    if (count)
        print_port_list(&out->sysscan->ports, Templ_UDP, fp);

    count = count_type(&out->sysscan->ports, Templ_SCTP);
    fprintf(fp, ") SCTP(%u;", count);
    if (count)
        print_port_list(&out->sysscan->ports, Templ_SCTP, fp);
        ...
        const unsigned char *px, unsigned length)
{
    ...
    fprintf(fp, "\tService: %s", sysscan_app_to_string(proto));
