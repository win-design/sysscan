...
#include "sysscan-status.h"     /* open or closed */
...
struct ThreadPair {
     ...
     * thread-pair. This is an index into the "sysscan->nic[]"
     * array.
     *
 ...
 ***************************************************************************/
static void
get_sources(const struct sysscan *sysscan,
            unsigned nic_index,
            unsigned *src_ip,
...
{
    const struct Source *src = &sysscan->nic[nic_index].src;
...
transmit_thread(void *v) /*aka. scanning_thread() */
{
    ...
    uint64_t start;
    uint64_t end;
    const struct sysscan *sysscan = parms->sysscan;
    unsigned retries = sysscan->retries;
    unsigned rate = (unsigned)sysscan->max_rate;
    unsigned r = retries + 1;
    uint64_t range;
    struct BlackRock blackrock;
    uint64_t count_ips = rangelist_count(&sysscan->targets);
    ...
    unsigned increment = (sysscan->shard.of-1) + sysscan->nic_count;
    unsigned src_ip;
    unsigned src_ip_mask;
    unsigned src_port;
    unsigned src_port_mask;
    ...
    *status_syn_count = 0;
    ...
    get_sources(sysscan, parms->nic_index,
                &src_ip, &src_ip_mask,
     ...
     * --max-rate parameter */
    throttler_start(throttler, sysscan->max_rate/sysscan->nic_count);
...
infinite:
     ...
     * ports */
    range = rangelist_count(&sysscan->targets)
            * rangelist_count(&sysscan->ports);
    ...
    start = sysscan->resume.index + (sysscan->shard.one-1) + parms->nic_index;
    end = range;
    ...
    for (i=start; i<end; ) {
        ...
        while (batch_size && i < end) {
            ...
            ip_them = rangelist_pick2(&sysscan->targets, xXx % count_ips, picker);
            port_them = rangelist_pick(&sysscan->ports, xXx / count_ips);
...
//printf("0x%08x 0x%08x 0x%04x 0x%08x 0x%04x    \n", cookie, ip_them, port_them, ip_me, port_me);
     ...
     */
    if (sysscan->is_infinite && !is_tx_done) {
        seed++;
        repeats++;
        goto infinite;
    }
 ...
 ***************************************************************************/
static unsigned
is_nic_port(const struct sysscan *sysscan, unsigned ip)
{
    unsigned i;
    for (i=0; i<sysscan->nic_count; i++)
        if (is_my_port(&sysscan->nic[i].src, ip))
            return 1;
    return 0;
}
...
receive_thread(void *v)
{
    ...
    *status_tcb_count = 0;
     ...
     */
    if (sysscan->pcap_filename[0]) {
        pcapfile = pcapfile_openwrite(sysscan->pcap_filename, 1);
    }
     ...
     */
    out = output_create(sysscan, parms->nic_index);
    ...
    if (sysscan->is_banners) {
        ...
        tcpcon = tcpcon_create_table(
            (size_t)((sysscan->max_rate/5) / sysscan->nic_count),
            parms->transmit_queue,
            parms->packet_buffers,
        ...
        tcpcon_set_banner_flags(tcpcon,
                sysscan->is_capture_cert,
                sysscan->is_capture_html,
                sysscan->is_capture_heartbleed,
				sysscan->is_capture_ticketbleed);
        if (sysscan->http_user_agent_length)
            tcpcon_set_parameter(   tcpcon,
                                    "http-user-agent",
                                    sysscan->http_user_agent_length,
                                    sysscan->http_user_agent);
        if (sysscan->is_heartbleed)
            tcpcon_set_parameter(   tcpcon,
                                 "heartbleed",
                                 1,
                                 "1");
        if (sysscan->is_ticketbleed)
            tcpcon_set_parameter(   tcpcon,
                                 "ticketbleed",
                                 1,
                                 "1");
        if (sysscan->is_poodle_sslv3)
            tcpcon_set_parameter(   tcpcon,
                                 "sslv3",
                                 1,
                                 "1");
        if (sysscan->tcp_connection_timeout) {
            char foo[64];
            sprintf_s(foo, sizeof(foo), "%u", sysscan->tcp_connection_timeout);
            tcpcon_set_parameter(   tcpcon,
                                 "timeout",
                                 strlen(foo),
                                 foo);
        }
        if (sysscan->tcp_hello_timeout) {
            char foo[64];
            sprintf_s(foo, sizeof(foo), "%u", sysscan->tcp_connection_timeout);
            tcpcon_set_parameter(   tcpcon,
                                 "hello-timeout",
        ...
        switch (parsed.found) {
            case FOUND_ARP:
                ...
                case 2: /* response */
                    ...
                    if (!rangelist_is_contains(&sysscan->targets, ip_them))
                        break;
            ...
            case FOUND_UDP:
            case FOUND_DNS:
                if (!is_nic_port(sysscan, port_me))
                    continue;
                if (parms->sysscan->nmap.packet_trace)
 ...
 ***************************************************************************/
static int
main_scan(struct sysscan *sysscan)
{
     ...
     */
    if (sysscan->script.name) {
        unsigned i;
        script = script_lookup(sysscan->script.name);
        
        /* If no ports specified on command-line, grab default ports */
        if (rangelist_count(&sysscan->ports) == 0)
            rangelist_parse_ports(&sysscan->ports, script->ports, 0);
        
        /* Kludge: change normal port range to script range */
        for (i=0; i<sysscan->ports.count; i++) {
            struct Range *r = &sysscan->ports.list[i];
    ...
    range = count_ips * count_ports + (uint64_t)(sysscan->retries * sysscan->max_rate);

    /*
     * If doing an ARP scan, then don't allow port scanning
     */
    if (rangelist_is_contains(&sysscan->ports, Templ_ARP)) {
        if (sysscan->ports.count != 1) {
     ...
     */
    if (count_ips > 1000000000ULL && rangelist_count(&sysscan->exclude_ip) == 0) {
     ...
     */
    payloads_trim(sysscan->payloads, &sysscan->ports);
    ...
    picker = rangelist_pick2_create(&sysscan->targets);
  ...
  __AFL_INIT();
     ...
     */
    for (index=0; index<sysscan->nic_count; index++) {
        struct ThreadPair *parms = &parms_array[index];
        int err;

        parms->sysscan = sysscan;
        parms->nic_index = index;
        parms->picker = picker;
        parms->my_index = sysscan->resume.index;
         ...
         */
        err = sysscan_initialize_adapter(
                            sysscan,
                            index,
                            parms->adapter_mac,
                            parms->router_mac
        ...
        if (err != 0)
            exit(1);
        parms->adapter = sysscan->nic[index].adapter;
        if (sysscan->nic[index].src.ip.range == 0) {
        ...
        template_packet_init(
                    ...
                    parms->router_mac,
                    sysscan->payloads,
                    rawsock_datalink(sysscan->nic[index].adapter),
                    sysscan->seed);

        /*
         * Set the "source port" of everything we transmit.
         */
        if (sysscan->nic[index].src.port.range == 0) {
            unsigned port = 40000 + now % 20000;
            sysscan->nic[index].src.port.first = port;
            sysscan->nic[index].src.port.last = port;
            sysscan->nic[index].src.port.range = 1;
        }

        parms->src = sysscan->nic[index].src;
         ...
         */
        if (sysscan->nmap.ttl)
            template_set_ttl(parms->tmplset, sysscan->nmap.ttl);

        if (sysscan->nic[0].is_vlan)
            template_set_vlan(parms->tmplset, sysscan->nic[0].vlan_id);
...
#define BUFFER_COUNT 16384
     ...
     */
    {
        ...
        LOG(0, "\nStarting sysscan version 1.0.1 -> r00t " sysscan_VERSION " (http://goo.gl/1y9cLW) at %s\n", buffer);
    ...
    status_start(&status);
    status.is_infinite = sysscan->is_infinite;
    while (!is_tx_done && sysscan->output.is_status_updates) {
        unsigned i;
        double rate = 0;
        ...
        if (min_index >= range && !sysscan->is_infinite) {
         ...
         */
        if (sysscan->output.is_status_updates)
    ...
    for (;;) {
        ...
        if (time(0) - now >= sysscan->wait)
            is_rx_done = 1;

        if (sysscan->output.is_status_updates) {
            status_print(&status, min_index, range, rate,
                total_tcbs, total_synacks, total_syns,
                sysscan->wait - (time(0) - now));
            ...
            if (transmit_count < sysscan->nic_count)
                continue;
            is_tx_done = 1;
            is_rx_done = 1;
            if (receive_count < sysscan->nic_count)
                continue;
...
int main(int argc, char *argv[])
{
     ...
     */
    memset(sysscan, 0, sizeof(*sysscan));
    sysscan->blackrock_rounds = 4;
    sysscan->output.is_show_open = 1; /* default: show syn-ack, not rst */
    sysscan->output.is_status_updates = 1; /* default: show status updates */
    sysscan->seed = get_entropy(); /* entropy for randomness */
    sysscan->wait = 10; /* how long to wait for responses when done */
    sysscan->max_rate = 100.0; /* max rate = hundred packets-per-second */
    sysscan->nic_count = 1;
    sysscan->shard.one = 1;
    sysscan->shard.of = 1;
    sysscan->min_packet_size = 60;
     ...
     */
#if !defined(WIN32)
    if (!sysscan->is_readscan) {
        if (access("/etc/sysscan/sysscan.conf", 0) == 0) {
            sysscan_read_config_file(sysscan, "/etc/sysscan/sysscan.conf");
        }
    }
#endif
     ...
     */
    sysscan_command_line(sysscan, argc, argv);
     ...
     */
    {
        uint64_t range = rangelist_count(&sysscan->targets) * rangelist_count(&sysscan->ports);
        uint64_t range2;
        rangelist_exclude(&sysscan->targets, &sysscan->exclude_ip);
        rangelist_exclude(&sysscan->ports, &sysscan->exclude_port);
        //rangelist_remove_range2(&sysscan->targets, range_parse_ipv4("224.0.0.0/4", 0, 0));

        range2 = rangelist_count(&sysscan->targets) * rangelist_count(&sysscan->ports);
        ...
        if (range2 != range && sysscan->resume.index) {
    ...
    case Operation_DebugIF:
        for (i=0; i<sysscan->nic_count; i++)
            rawsock_selftest_if(sysscan->nic[i].ifname);
        return 0;

    case Operation_ReadRange:
        main_readrange(sysscan);
        return 0;
    ...
    case Operation_ReadScan:
        {
             ...
             */
            read_binary_scanfile(sysscan, start, stop, argv);
    ...
    case Operation_Benchmark:
        ...
        blackrock_benchmark(sysscan->blackrock_rounds);
        blackrock2_benchmark(sysscan->blackrock_rounds);
        smack_benchmark();
        exit(1);
        break;
