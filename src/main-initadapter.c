 ...
 ***************************************************************************/
int
sysscan_initialize_adapter(
    struct sysscan *sysscan,
    unsigned index,
    ...
    )
{
     ...
     */
    if (sysscan->nic[index].ifname && sysscan->nic[index].ifname[0])
        ifname = sysscan->nic[index].ifname;
    else {
     ...
     */
    adapter_ip = sysscan->nic[index].src.ip.first;
    if (adapter_ip == 0) {
        ...
        sysscan->nic[index].src.ip.first = adapter_ip;
        sysscan->nic[index].src.ip.last = adapter_ip;
        sysscan->nic[index].src.ip.range = 1;
    }
    if (adapter_ip == 0) {
     ...
     */
    memcpy(adapter_mac, sysscan->nic[index].my_mac, 6);
     ...
     * turn it on.
     */
    sysscan->nic[index].adapter = rawsock_init_adapter(
                                            ifname,
                                            sysscan->is_pfring,
                                            sysscan->is_sendq,
                                            sysscan->nmap.packet_trace,
                                            sysscan->is_offline,
                                            (void*)sysscan->bpf_filter,
                                            sysscan->nic[index].is_vlan,
                                            sysscan->nic[index].vlan_id);
    if (sysscan->nic[index].adapter == 0) {
        fprintf(stderr, "adapter[%s].init: failed\n", ifname);
        return -1;
    }
    LOG(3, "rawsock: ignoring transmits\n");
    rawsock_ignore_transmits(sysscan->nic[index].adapter, adapter_mac);
