           ...
           const struct RangeList *btypes)
{
    ...
    if (memcmp(buf, "sysscan/1.1", 11) != 0) {
        fprintf(stderr,
                "%s: unknown file format (expeced \"sysscan/1.1\")\n",
                filename);
        goto end;
    }
...
/*****************************************************************************
 * When sysscan is called with the "--readscan" parameter, it doesn't
 ...
 *****************************************************************************/
void
read_binary_scanfile(struct sysscan *sysscan,
                     int arg_first, int arg_max, char *argv[])
{
     ...
     * looks like:
     *   sysscan --foo --readscan file1.scan file2.scan --bar
     * Then arg_first=3 and arg_max=5.
     */
    for (i=arg_first; i<arg_max; i++) {
        parse_file(out, argv[i], &sysscan->targets, &sysscan->ports,
                   &sysscan->banner_types);
    }
