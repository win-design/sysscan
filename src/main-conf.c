/*
    Read in the configuration for sysscan.
...
print_version()
{
    ...
    printf("\n");
    printf("sysscan version %s ( %s )\n", 
        sysscan_VERSION,
        "https://github.com/robertdavidgraham/sysscan"
        );
...
{
    printf("sysscan (https://github.com/robertdavidgraham/sysscan)\n"
"Usage: sysscan [Options] -p{Target-Ports} {Target-IP-Ranges}\n"
...
"EXAMPLES:\n"
"  sysscan -v -sS 192.168.0.0/16 10.0.0.0/8 -p 80\n"
"  sysscan 23.0.0.0/0 -p80 --banners -output-format binary --output-filename internet.scan\n"
"  sysscan --open --banners --readscan internet.scan -oG internet_scan.grepable\n"
"SEE (https://github.com/robertdavidgraham/sysscan) FOR MORE HELP\n"
"\n");
}
 ...
 ***************************************************************************/
static void
sysscan_echo_nic(struct sysscan *sysscan, FILE *fp, unsigned i)
{
    char zzz[64];
    ...
    if (sysscan->nic_count <= 1)
        zzz[0] = '\0';
    else
        sprintf_s(zzz, sizeof(zzz), "[%u]", i);

    fprintf(fp, "adapter%s = %s\n", zzz, sysscan->nic[i].ifname);
    if (sysscan->nic[i].src.ip.first == sysscan->nic[i].src.ip.last)
        fprintf(fp, "adapter-ip%s = %u.%u.%u.%u\n", zzz,
            (sysscan->nic[i].src.ip.first>>24)&0xFF,
            (sysscan->nic[i].src.ip.first>>16)&0xFF,
            (sysscan->nic[i].src.ip.first>> 8)&0xFF,
            (sysscan->nic[i].src.ip.first>> 0)&0xFF
            );
    else
        fprintf(fp, "adapter-ip%s = %u.%u.%u.%u-%u.%u.%u.%u\n", zzz,
            ...
            (sysscan->nic[i].src.ip.first>> 8)&0xFF,
            (sysscan->nic[i].src.ip.first>> 0)&0xFF,
            (sysscan->nic[i].src.ip.last>>24)&0xFF,
            (sysscan->nic[i].src.ip.last>>16)&0xFF,
            (sysscan->nic[i].src.ip.last>> 8)&0xFF,
            (sysscan->nic[i].src.ip.last>> 0)&0xFF
            );

    fprintf(fp, "adapter-mac%s = %02x:%02x:%02x:%02x:%02x:%02x\n", zzz,
            sysscan->nic[i].my_mac[0],
            sysscan->nic[i].my_mac[1],
            sysscan->nic[i].my_mac[2],
            sysscan->nic[i].my_mac[3],
            sysscan->nic[i].my_mac[4],
            sysscan->nic[i].my_mac[5]);
    if (sysscan->nic[i].router_ip) {
        fprintf(fp, "router-ip%s = %u.%u.%u.%u\n", zzz,
            (sysscan->nic[i].router_ip>>24)&0xFF,
            (sysscan->nic[i].router_ip>>16)&0xFF,
            (sysscan->nic[i].router_ip>> 8)&0xFF,
            (sysscan->nic[i].router_ip>> 0)&0xFF
            );
    } else
    fprintf(fp, "router-mac%s = %02x:%02x:%02x:%02x:%02x:%02x\n", zzz,
            sysscan->nic[i].router_mac[0],
            sysscan->nic[i].router_mac[1],
            sysscan->nic[i].router_mac[2],
            sysscan->nic[i].router_mac[3],
            sysscan->nic[i].router_mac[4],
            sysscan->nic[i].router_mac[5]);
 ...
 ***************************************************************************/
static void
sysscan_echo(struct sysscan *sysscan, FILE *fp)
{
    unsigned i;
    unsigned l = 0;
    ...
    fprintf(fp, "shard = %u/%u\n", sysscan->shard.one, sysscan->shard.of);
    if (sysscan->is_banners)
        fprintf(fp, "banners = true\n");

    fprintf(fp, "# ADAPTER SETTINGS\n");
    if (sysscan->nic_count == 0)
        sysscan_echo_nic(sysscan, fp, 0);
    else {
        for (i=0; i<sysscan->nic_count; i++)
            sysscan_echo_nic(sysscan, fp, i);
    }
    ...
    case Output_Redis:
        ...
        fprintf(fp, "redis = %u.%u.%u.%u:%u\n",
            (unsigned char)(sysscan->redis.ip>>24),
            (unsigned char)(sysscan->redis.ip>>16),
            (unsigned char)(sysscan->redis.ip>> 8),
            (unsigned char)(sysscan->redis.ip>> 0),
            sysscan->redis.port);
        break;

    default:
        fprintf(fp, "output-format = unknown(%u)\n", sysscan->output.format);
        break;
    }
    fprintf(fp, "show = %s,%s,%s\n",
            sysscan->output.is_show_open?"open":"",
            sysscan->output.is_show_closed?"closed":"",
            sysscan->output.is_show_host?"host":""
            );
    if (!sysscan->output.is_show_open)
        fprintf(fp, "noshow = open\n");
    fprintf(fp, "output-filename = %s\n", sysscan->output.filename);
    if (sysscan->output.is_append)
        fprintf(fp, "output-append = true\n");
    fprintf(fp, "rotate = %u\n", sysscan->output.rotate.timeout);
    fprintf(fp, "rotate-dir = %s\n", sysscan->output.rotate.directory);
    fprintf(fp, "rotate-offset = %u\n", sysscan->output.rotate.offset);
    fprintf(fp, "rotate-filesize = %" PRIu64 "\n", sysscan->output.rotate.filesize);
    fprintf(fp, "pcap = %s\n", sysscan->pcap_filename);
    ...
    l = 0;
    for (i=0; i<sysscan->ports.count; i++) {
        struct Range range = sysscan->ports.list[i];
        do {
    ...
    fprintf(fp, "\n");
    for (i=0; i<sysscan->targets.count; i++) {
        struct Range range = sysscan->targets.list[i];
        fprintf(fp, "range = ");
        fprintf(fp, "%u.%u.%u.%u",
            (range.begin>>24)&0xFF,
            (range.begin>>16)&0xFF,
    ...
    fprintf(fp, "\n");
    if (sysscan->http_user_agent)
        fprintf(    fp,
                "http-user-agent = %.*s\n",
                sysscan->http_user_agent_length,
                sysscan->http_user_agent);

    for (i=0; i<sizeof(sysscan->http_headers)/sizeof(sysscan->http_headers[0]); i++) {
        if (sysscan->http_headers[i].header_name == 0)
            continue;
        fprintf(    fp,
                    "http-header[%s] = %.*s\n",
                    sysscan->http_headers[i].header_name,
                    sysscan->http_headers[i].header_value_length,
                sysscan->http_headers[i].header_value);
    }


    fprintf(fp, "%scapture = cert\n", sysscan->is_capture_cert?"":"no");
    fprintf(fp, "%scapture = html\n", sysscan->is_capture_html?"":"no");
    fprintf(fp, "%scapture = heartbleed\n", sysscan->is_capture_heartbleed?"":"no");
    fprintf(fp, "%scapture = ticketbleed\n", sysscan->is_capture_ticketbleed?"":"no");

    /*
     *  TCP payloads
     */
    fprintf(fp, "\n");
    fprintf(fp, "min-packet = %u\n", sysscan->min_packet_size);

    {
        struct TcpCfgPayloads *pay;
        for (pay = sysscan->tcp_payloads; pay; pay = pay->next) {
 ...
 ***************************************************************************/
void
sysscan_save_state(struct sysscan *sysscan)
{
    char filename[512];
    FILE *fp;
    int err;
    ...
    fprintf(fp, "\n# resume information\n");
    fprintf(fp, "resume-index = %" PRIu64 "\n", sysscan->resume.index);

    sysscan_echo(sysscan, fp);
 ...
 ***************************************************************************/
static void
sysscan_set_parameter(struct sysscan *sysscan,
                      const char *name, const char *value)
{
    ...
    if (EQUALS("conf", name) || EQUALS("config", name)) {
        sysscan_read_config_file(sysscan, value);
    } else if (EQUALS("adapter", name) || EQUALS("if", name) || EQUALS("interface", name)) {
        if (sysscan->nic[index].ifname[0]) {
            fprintf(stderr, "CONF: overwriting \"adapter=%s\"\n", sysscan->nic[index].ifname);
        }
        if (sysscan->nic_count < index + 1)
            sysscan->nic_count = index + 1;
        sprintf_s(  sysscan->nic[index].ifname,
                    sizeof(sysscan->nic[index].ifname),
                    "%s",
                    value);
        ...
        sysscan->nic[index].src.ip.first = range.begin;
        sysscan->nic[index].src.ip.last = range.end;
        sysscan->nic[index].src.ip.range = (uint64_t)range.end - range.begin + 1;
        ...
        sysscan->nic[index].src.port.first = ports.list[0].begin;
        sysscan->nic[index].src.port.last = ports.list[0].end;
        sysscan->nic[index].src.port.range = ports.list[0].end - ports.list[0].begin + 1;
        ...
        if (memcmp(sysscan->nic[index].my_mac, mac, 6) == 0)
            return;

        /* Warn if we are overwriting a Mac address */
        if (sysscan->nic[index].my_mac_count != 0) {
            LOG(0, "WARNING: overwriting MAC address\n");
        }

        memcpy(sysscan->nic[index].my_mac, mac, 6);
        sysscan->nic[index].my_mac_count = 1;
    }
        ...
        memcpy(sysscan->nic[index].router_mac, mac, 6);
    }
        ...
        sysscan->nic[index].router_ip = range.begin;
    }
        ...
        unsigned is_error = 0;
        rangelist_parse_ports(&sysscan->ports, value, &is_error);
        if (sysscan->op == 0)
            sysscan->op = Operation_Scan;
    }
        ...
        app = sysscan_string_to_app(value);
        
        if (app)
            rangelist_add_range(&sysscan->banner_types, app, app);
        else {
        ...
        unsigned is_error = 0;
        rangelist_parse_ports(&sysscan->exclude_port, value, &is_error);
        if (is_error) {
        ...
        range.end = Templ_ARP;
        rangelist_add_range(&sysscan->ports, range.begin, range.end);
        sysscan_set_parameter(sysscan, "router-mac", "ff-ff-ff-ff-ff-ff");
        sysscan->is_arp = 1; /* needs additional flag */
        LOG(5, "--arpscan\n");
    } else if (EQUALS("bpf", name)) {
        size_t len = strlen(value) + 1;
        if (sysscan->bpf_filter)
            free(sysscan->bpf_filter);
        sysscan->bpf_filter = (char*)malloc(len);
        memcpy(sysscan->bpf_filter, value, len);
    } else if (EQUALS("capture", name)) {
        if (EQUALS("cert", value))
            sysscan->is_capture_cert = 1;
        else if (EQUALS("html", value))
            sysscan->is_capture_html = 1;
        else if (EQUALS("heartbleed", value))
            sysscan->is_capture_heartbleed = 1;
        else if (EQUALS("ticketbleed", value))
            sysscan->is_capture_ticketbleed = 1;
        else {
        ...
        if (EQUALS("cert", value))
            sysscan->is_capture_cert = 0;
        else if (EQUALS("html", value))
            sysscan->is_capture_html = 0;
        else if (EQUALS("heartbleed", value))
            sysscan->is_capture_heartbleed = 0;
        else if (EQUALS("ticketbleed", value))
            sysscan->is_capture_ticketbleed = 0;
        else {
            fprintf(stderr, "FAIL: %s: unknown capture type\n", value);
            exit(1);
        }
        ...
        for (;;) {
            ...
            rangelist_add_range(&sysscan->targets, range.begin, range.end);
        ...
        for (;;) {
            ...
            rangelist_add_range(&sysscan->exclude_ip, range.begin, range.end);
        ...
        sysscan_echo(sysscan, stdout);
        exit(0);
    } else if (EQUALS("excludefile", name)) {
        unsigned count1 = sysscan->exclude_ip.count;
        unsigned count2;
        LOG(1, "EXCLUDING: %s\n", value);
        ranges_from_file(&sysscan->exclude_ip, value);
        count2 = sysscan->exclude_ip.count;
        if (count2 - count1)
        fprintf(stderr, "%s: excluding %u ranges from file\n",
                value, count2 - count1);
    } else if (EQUALS("heartbleed", name)) {
        sysscan->is_heartbleed = 1;
        sysscan_set_parameter(sysscan, "no-capture", "cert");
        sysscan_set_parameter(sysscan, "no-capture", "heartbleed");
        sysscan_set_parameter(sysscan, "banners", "true");
    } else if (EQUALS("ticketbleed", name)) {
        sysscan->is_ticketbleed = 1;
        sysscan_set_parameter(sysscan, "no-capture", "cert");
        sysscan_set_parameter(sysscan, "no-capture", "ticketbleed");
        sysscan_set_parameter(sysscan, "banners", "true");
        ...
        sysscan_set_parameter(sysscan, foo, buf2);
        ...
        pay->port = index;
        pay->next = sysscan->tcp_payloads;
        sysscan->tcp_payloads = pay;
        ...
        if (sysscan->http_user_agent)
            free(sysscan->http_user_agent);
        sysscan->http_user_agent_length = (unsigned)strlen(value);
        sysscan->http_user_agent = (unsigned char *)malloc(sysscan->http_user_agent_length+1);
        memcpy( sysscan->http_user_agent,
                value,
                sysscan->http_user_agent_length+1
                );
        ...
        for (j=0; j < sizeof(sysscan->http_headers)/sizeof(sysscan->http_headers[0]); j++) {
            if (sysscan->http_headers[j].header_name == 0) {
                sysscan->http_headers[j].header_name = newname;
                sysscan->http_headers[j].header_value = newvalue;
                sysscan->http_headers[j].header_value_length = value_length;
                return;
            }
        }

    } else if (EQUALS("iflist", name)) {
        sysscan->op = Operation_List_Adapters;
    } else if (EQUALS("includefile", name)) {
        ranges_from_file(&sysscan->targets, value);
    } else if (EQUALS("infinite", name)) {
        sysscan->is_infinite = 1;
    } else if (EQUALS("interactive", name)) {
        sysscan->output.is_interactive = 1;
    } else if (EQUALS("nointeractive", name)) {
        sysscan->output.is_interactive = 0;
    } else if (EQUALS("status", name)) {
        sysscan->output.is_status_updates = 1;
    } else if (EQUALS("nostatus", name)) {
        sysscan->output.is_status_updates = 0;
        ...
        sysscan->min_packet_size = (unsigned)parseInt(value);
    } else if (EQUALS("max-retries", name)) {
        sysscan_set_parameter(sysscan, "retries", value);
    } else if (EQUALS("max-rate", name)) {
        sysscan_set_parameter(sysscan, "rate", value);
        ...
        if (sysscan->payloads == NULL)
            sysscan->payloads = payloads_create();
        payloads_read_pcap(value, sysscan->payloads);
        ...
        if (err || fp == NULL) {
            ...
            if (sysscan->payloads == NULL)
                sysscan->payloads = payloads_create();
            payloads_read_file(fp, value, sysscan->payloads);
            fclose(fp);
        }
    } else if (EQUALS("offline", name)) {
        /* Run in "offline" mode where it thinks it's sending packets, but
         * it's not */
        sysscan->is_offline = 1;
    } else if (EQUALS("open", name) || EQUALS("open-only", name)) {
        sysscan->output.is_show_open = 1;
        sysscan->output.is_show_closed = 0;
        sysscan->output.is_show_host = 0;
        ...
        for (;;) {
            ...
            if (EQUALSx("open", val2, val2_len))
                ...
                sysscan->output.is_show_closed = 1;
            else if (EQUALSx("open", val2, val2_len))
                sysscan->output.is_show_host = 1;
        ...
        sysscan->output.format = x;
    } else if (EQUALS("output-filename", name) || EQUALS("output-file", name)) {
        if (sysscan->output.format == 0)
            sysscan->output.format = Output_XML;
        strcpy_s(sysscan->output.filename,
                 sizeof(sysscan->output.filename), 
                 value);
    } else if (EQUALS("pcap", name)) {
        strcpy_s(sysscan->pcap_filename, sizeof(sysscan->pcap_filename), value);
    } else if (EQUALS("packet-trace", name) || EQUALS("trace-packet", name)) {
        sysscan->nmap.packet_trace = 1;
        ...
        sysscan->op = Operation_ReadRange;
    } else if (EQUALS("reason", name)) {
        sysscan->output.is_reason = 1;
        ...
        sysscan->redis.ip = range.begin;
        sysscan->redis.port = port;
        sysscan->output.format = Output_Redis;
        strcpy_s(sysscan->output.filename, 
                 sizeof(sysscan->output.filename), 
                 "<redis>");
        ...
        sysscan_read_config_file(sysscan, value);
        sysscan_set_parameter(sysscan, "output-append", "true");
    } else if (EQUALS("resume-index", name)) {
        sysscan->resume.index = parseInt(value);
    } else if (EQUALS("resume-count", name)) {
        sysscan->resume.count = parseInt(value);
        ...
        sysscan->output.rotate.timeout = (unsigned)parseTime(value);
    } else if (EQUALS("rotate-offset", name) || EQUALS("ouput-rotate-offset", name)) {
        sysscan->output.rotate.offset = (unsigned)parseTime(value);
    } else if (EQUALS("rotate-size", name) || EQUALS("rotate-filesize", name)) {
        sysscan->output.rotate.filesize = parseSize(value);
    } else if (EQUALS("rotate-dir", name) || EQUALS("rotate-directory", name) || EQUALS("ouput-rotate-dir", name)) {
        char *p;
        strcpy_s(   sysscan->output.rotate.directory,
                    sizeof(sysscan->output.rotate.directory),
                    value);

        /* strip trailing slashes */
        p = sysscan->output.rotate.directory;
        while (*p && (p[strlen(p)-1] == '/' || p[strlen(p)-1] == '/'))
            p[strlen(p)-1] = '\0';
    } else if (EQUALS("script", name)) {
        if (EQUALS("heartbleed", value)) {
            sysscan_set_parameter(sysscan, "heartbleed", "true");
            return;
		} else if (EQUALS("ticketbleed", value)) {
            sysscan_set_parameter(sysscan, "ticketbleed", "true");
            return;
        } else if (EQUALS("poodle", value) || EQUALS("sslv3", value)) {
            sysscan->is_poodle_sslv3 = 1;
            sysscan_set_parameter(sysscan, "no-capture", "cert");
            sysscan_set_parameter(sysscan, "banners", "true");
            return;
        }
        ...
        if (sysscan->script.name != NULL) {
            if (strcmp(sysscan->script.name, value) == 0)
                return; /* ok */
            else {
                fprintf(stderr, "FAIL: only one script supported at a time\n");
                fprintf(stderr, "  hint: '%s' is existing script, '%s' is new script\n",
                        sysscan->script.name, value);
                exit(1);
            }
        }
        
        sysscan->script.name = script_lookup(value)->name;
        ...
        sysscan->op = Operation_Selftest;
        return;
    } else if (EQUALS("benchmark", name)) {
        sysscan->op = Operation_Benchmark;
        return;
    } else if (EQUALS("source-port", name) || EQUALS("sourceport", name)) {
        sysscan_set_parameter(sysscan, "adapter-port", value);
        ...
        sysscan->shard.one = one;
        sysscan->shard.of = of;

    } else if (EQUALS("nobacktrace", name) || EQUALS("backtrace", name)) {
        ;
    } else if (EQUALS("no-stylesheet", name)) {
        sysscan->output.stylesheet[0] = '\0';
    } else if (EQUALS("stylesheet", name)) {
        strcpy_s(sysscan->output.stylesheet, 
                 sizeof(sysscan->output.stylesheet), 
                 value);
        ...
        if (EQUALS("csv", value))
            sysscan->is_test_csv = 1;
    } else if (EQUALS("notest", name)) {
        if (EQUALS("csv", value))
            sysscan->is_test_csv = 0;
        ...
        sysscan->nic[index].is_vlan = 1;
        sysscan->nic[index].vlan_id = (unsigned)parseInt(value);
    } else if (EQUALS("wait", name)) {
        if (EQUALS("forever", value))
            sysscan->wait =  INT_MAX;
        else
            sysscan->wait = (unsigned)parseInt(value);
    } else if (EQUALS("webxml", name)) {
        sysscan_set_parameter(sysscan, "stylesheet", "http://nmap.org/svn/docs/nmap.xsl");
    } else {
...
{
    printf(
"sysscan is a fast port scanner. The primary input parameters are the\n"
"IP addresses/ranges you want to scan, and the port numbers. An example\n"
"is the following, which scans the 10.x.x.x network for web servers:\n"
" sysscan 10.0.0.0/8 -p80\n"
 ...
 ***************************************************************************/
void
sysscan_command_line(struct sysscan *sysscan, int argc, char *argv[])
{
    int i;
    ...
    for (i=1; i<argc; i++) {
        ...
        if (argv[i][0] == '-' && argv[i][1] == '-') {
            if (strcmp(argv[i], "--help") == 0) {
                ...
                sysscan_set_parameter(sysscan, name2, value);
            }
            continue;
        }
        ...
        if (argv[i][0] == '-') {
            ...
            case 'c':
                if (argv[i][2])
                    ...
                    arg = argv[++i];
                sysscan_read_config_file(sysscan, arg);
                break;
            ...
            case 'e':
                if (argv[i][2])
                    ...
                    arg = argv[++i];
                sysscan_set_parameter(sysscan, "adapter", arg);
                break;
            case 'f':
                fprintf(stderr, "nmap(%s): fragmentation not yet supported\n", argv[i]);
                exit(1);
            ...
            case 'g':
                if (argv[i][2])
                    ...
                    arg = argv[++i];
                sysscan_set_parameter(sysscan, "adapter-port", arg);
                break;
            case 'h':
            case '?':
                sysscan_usage();
            ...
            case 'o': /* nmap output format */
                switch (argv[i][2]) {
                case 'A':
                    sysscan->output.format = Output_All;
                    fprintf(stderr, "nmap(%s): unsupported output format\n", argv[i]);
                    exit(1);
                    break;
                case 'B':
                    sysscan->output.format = Output_Binary;
                    break;
                case 'J':
                    sysscan->output.format = Output_JSON;
                    break;
                case 'N':
                    sysscan->output.format = Output_Nmap;
                    fprintf(stderr, "nmap(%s): unsupported output format\n", argv[i]);
                    exit(1);
                    break;
                case 'X':
                ...
                case 'R':
                    ...
                    if (i+1 < argc && argv[i+1][0] != '-')
                        sysscan_set_parameter(sysscan, "redis", argv[i+1]);
                    break;
                case 'S':
                    sysscan->output.format = Output_ScriptKiddie;
                    fprintf(stderr, "nmap(%s): unsupported output format\n", argv[i]);
                    exit(1);
                    break;
                case 'G':
                    sysscan->output.format = Output_Grepable;
                    break;
                case 'L':
                    sysscan_set_parameter(sysscan, "output-format", "list");
                    break;
                case 'U':
                    sysscan_set_parameter(sysscan, "output-format", "unicornscan");
                    break;
                default:
                ...
                sysscan_set_parameter(sysscan, "output-filename", argv[i]);
                break;
            case 'O':
		...
		//			fprintf(stderr, "%.*s: empty parameter\n", argv[0], argv[1]);
                sysscan_set_parameter(sysscan, "ports", arg);
                break;
            case 'P':
                switch (argv[i][2]) {
                case 'n':
            ...
            case 'S':
                if (argv[i][2])
                    ...
                    arg = argv[++i];
                sysscan_set_parameter(sysscan, "adapter-ip", arg);
                break;
            case 'v':
                {
                    int v;
            ...
            case 'V': /* print version and exit */
                sysscan_set_parameter(sysscan, "version", "");
                break;
            case 'W':
                sysscan->op = Operation_List_Adapters;
                return;
         ...
         * IPv4 range
         */
        sysscan_set_parameter(sysscan, "range", argv[i]);
    }
}
 ...
 ***************************************************************************/
void
sysscan_read_config_file(struct sysscan *sysscan, const char *filename)
{
    FILE *fp;
    errno_t err;
    char line[65536];
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        *value = '\0';
        ...
        sysscan_set_parameter(sysscan, name, value);
    }
...
/***************************************************************************
 ***************************************************************************/
int sysscan_conf_contains(const char *x, int argc, char **argv)
{
    int i;
...
mainconf_selftest()
{
    ...
    /* */
    {
        ...
        if (sysscan_conf_contains("--nothing", argc, argv))
            return 1;

        if (!sysscan_conf_contains("--readscan", argc, argv))
            return 1;
    }
